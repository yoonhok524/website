...
Pub 사이트의 시작페이지에서는 Flutter와 호환되는 가장 인기있는 패키지들을 확인할 수 있고(일반적으로 Flutter 호환 가능하다고 종속성을 선언한 패키지), 패키지를 검색할 수도 있습니다. 
...
Pub에 게시되지 않은 패키지도 사용 가능합니다. 추가적인 의존성 옵션을 통해서, 일반 공개를 원하시지 않은 개인적인 플러그인이나 아직 게시할 준비가 되지 않은 패키지를 사용할 수 있습니다. 
...
* **Git** 종속성: Git 리포지토리에 저장 되어 있는 패키지에 대한 종속을 사용할 수도 있습니다. 패키지가 루트 리포지토리에 있다면 아래 구문을 사용하시면 됩니다. 
  ```
  dependencies:
    plugin1:
      git:
...
* 패키지의 폴더에 있는 **Git** 종속성: 기본적으로 Pub은 패키지가 Git 리포지토리의 루트 경로에 있다고 가정합니다. 만약 이 경우가 아니라면, `path` 인수를 이용해서 경로를 직접 지정해 줄 수 있습니다. 예: 
  ```
  dependencies:
    package1:
      git:
